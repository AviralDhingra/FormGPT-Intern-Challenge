...
function FillForm() {
  ...
  const handleSubmit = (event) => {
    ...
    post("/getNextQuestion", {
      ...
      setSquares((newSquare) => [
        ...newSquare,
        { resp: data["q"], redirect: data["redirect"], state: 0 },
      ]);
    });
  };
  ...
  useEffect(() => {
    if (id === "") {
      ...
      post("/getConversationID", { id: formId }).then((data) => {
        setId(data["id"]);
        setSquares((squares) => [
          ...squares,
          { resp: data["startMessage"], redirect: "", state: 0 },
        ]);
  ...
  return (
    <div
      className="container"
      style={{ backgroundColor: colors[0], position: "relative" }}
    >
      {logo == "" ? (
        ""
      ) : (
        <img src={logo} className="absolute top-0 left-0 m-4 w-16 h-16" />
      )}
      <Squares
        squares={squares}
            ...
            <form onSubmit={handleSubmit}>
              ...
              {square["state"] > 0 ? (
                <>
                  {square["state"] == 1 ? (
                    <button
                      className="btn text-white disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed shadow-none ml-3"
                      disabled
                    >
                      <svg
